/*
  # Data Wizard Schema Migration

  1. New Tables
    - file_types (id, name)
    - output_formats (id, name)
    - folders (id, user_id, name, timestamps)
    - history (id, user_id, input_file_type_id, output_format_id, etc)
    - output_files (id, history_id, name, path, size, etc)
    - charts (id, user_id, type, name, timestamp)
    - output_format_preferences (user_id, format, updated_at)

  2. Security
    - Enable RLS on all tables
    - Add policies for data access control
    - Public read access for file_types and output_formats
    - User-specific access for all other tables
...
CREATE TABLE IF NOT EXISTS history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  input_file_type_id INTEGER REFERENCES file_types(id),
